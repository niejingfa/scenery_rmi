  ...
  module Definition
    class ModuleDefinition < Struct.new(:namespaces, :name, :backend, :primary_key, :prefix, :description, :children, :load_from_remote)
      ...
      def to_object(container)
        ...
        if container == ::Scenery || container.name.to_s.starts_with?('Scenery::')
          # 老的Scenery访问形态
          const = Module.new do
          ...
          # 新的Backend名称开头的访问形态
          const = Module.new
          SceneryRmi::Manager.application.inject_load_from_remote(const, self.backend)
        end
        container.const_set(module_name, const)
        tags = container.name.underscore.split('/')
        tags.unshift 'rmi'
        tags << module_name.to_s
        SceneryLogger.assign_for(const, tags: tags)
        const
      end
